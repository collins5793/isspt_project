...
// Charger équipes
$stmt = $pdo->query("
    SELECT t.team_id, t.name, s.id_season
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $external_name = $_POST['external_name'] !== "" ? $_POST['external_name'] : null;
    $position = $_POST['position'];
    $shirt_number = $_POST['shirt_number'];

    $sql = "INSERT INTO football_players 
        (user_id, external_name, position, shirt_number, team_id, season_id)
        VALUES (?,?,?,?,?,?)";

    $stmt = $pdo->prepare($sql);
    $stmt->execute([$user_id, $external_name, $position, $shirt_number, $team_id, $season_id]);

    header("Location: players_list.php?success=1");
    exit;
}
?>
