...
if (!$epreuve) die("Épreuve introuvable.");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($ext !== 'pdf') $errors[] = "Seul le format PDF est autorisé.";
            ...
            if (!move_uploaded_file($_FILES['file']['tmp_name'], $file_path)) $errors[] = "Erreur lors de l'upload du fichier.";
        }
    }
...
<h2 class="page-title mb-4">✏️ Modifier l'épreuve : <?= htmlspecialchars($titre) ?></h2>
...
<form method="POST" enctype="multipart/form-data" class="form-card bg-white p-4 rounded shadow-sm">
